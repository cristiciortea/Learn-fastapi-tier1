...
#    docker build -t fastapi_learn:v0.4 . -f './docker/Dockerfile' --no-cache --build-arg ENV_STATE=dev --build-arg POETRY_VERSION=1.2.0
    docker build -t fastapi_learn:v0.4 . -f './docker/Dockerfile' --build-arg ENV_STATE=dev --build-arg POETRY_VERSION=1.2.0
}
...
function start_fastapi_app() {
    docker run --name fastapi_learn -p 8000:8000 -d --rm -it fastapi_learn:v0.4 /bin/bash -c 'poetry run uvicorn app.main:app --reload --host 0.0.0.0 --port 8000'
}
...
run_cmd build_fastapi_app_image
